...
const CLASSNAME ='playFabCheckDailyStatsStatus'
...
const _getLatestStatVersionsAsync = preventConcurrentExecution("_getLatestStatVersionsAsync", async () => {
  ...
  const result:PlayFabServerModels.GetPlayerStatisticVersionsResult = await PlayFabHelper.GetPlayerStatisticVersions({StatisticName:'coinsCollectedDaily'})
...
export function getLatestStatVersionsAsync(callback?:{onSuccess:()=>void,onFailure?:(e:any)=>void}): Promise<StatVersionCheckResult> {
  const promise: Promise<StatVersionCheckResult> = new Promise(async (resolve, reject) => {
    let loginRes: StatVersionCheckResult;
    try {
      //if(true){
      //  throw new Error("getLatestStatVersionsAsync testing failure")
      //}
      loginRes = await _getLatestStatVersionsAsync();
      resolve(loginRes);
    } catch (e) {
      console.log(CLASSNAME,"getLatestStatVersionsAsync failed. rejecting... ", e);
  ...
  promise.then(() => {
    if (callback && callback.onSuccess) {
      console.log(CLASSNAME,"getLatestStatVersionsAsync calling callback. onSuccess");
      callback.onSuccess();
    } else {
      console.log(CLASSNAME,"getLatestStatVersionsAsync success,no callback. onSuccess");
    }
    ...
    console.log(CLASSNAME,"getLatestStatVersionsAsync REJECTED :(",e);
  });
  return promise;
}