...
import { PlayerInventoryState } from "../state/server-state-spec";
    ...
    GetPlayerCombinedInfo(getPlayerCombinedInfoRequest).then(
      function(result:PlayFabServerModels.GetPlayerCombinedInfoResult){
        ...
        */
        ...
        if( updateStats.completedLevel && updateStats.place > 0 && Math.abs(updateStats.totalTime) < Math.abs(CONFIG.MAX_POSSIBLE_RACE_TIME)){
