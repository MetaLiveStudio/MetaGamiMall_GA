...
export class Dispenser extends Entity {
  ...
  constructor(
    ...
    if (visible) {
      this.addComponent(
        new GLTFShape(
          "bcebce0f-1873-4148-89bc-24c2ceb9b2cc/models/poap/POAP_dispenser.glb"
        )
      );
    }
    ...
    let button = new Entity("123");
    if (visible) {
      button.addComponent(
        new GLTFShape(
          "bcebce0f-1873-4148-89bc-24c2ceb9b2cc/models/poap/POAP_button.glb"
        )
      );
    }
            ...
            if (eventTimeDate.getTime() > now.getTime()) {
              ...
              try {
                ...
                if (dateFmt.indexOf(":", 15) > 0) {
                  dateFmt =
                    dateFmt.substr(0, dateFmt.indexOf(":", 15)) + " UTC";
                }
              } catch (e) {
                log("poap time check error", e);
              }
              UIdisplayAnnouncement(
  ...
  async makeTransaction(poapServer: string, event: string) {
    ...
    const url = poapServer; // 'https://www.metadoge.art/api/dcl/claim/poap'//'https://dev.metadoge.art/api/dcl/claim/poap'
    let method = "POST";
    ...
    try {
      ...
      //PARSE
      //json = await JSON.parse(response.text) //SIGNED FETCH VERSION
