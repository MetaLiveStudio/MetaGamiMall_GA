...
@Component("piano.OriginTransform")
 ...
 * @public
 */
@Component("piano.triggerComponentCE")
...
export class TriggerSystem implements ISystem {
  ...
  private constructor() {
    ...
    log(
      "TriggerSystem this._componentGroup " +
        this._componentGroup.entities.length
    );
  }
  ...
  private static removeTriggerFromSystem(wrapper: TriggerWrapper) {
    ...
    for (let i = 0; i < activeCollisions.length; i++) {
      ...
      if (
        ...
        (
          activeCollisions[i].trigger.onTriggerExit as (entity: IEntity) => void
        )(wrapper.entity);
...
class TriggerWrapper {
  ...
  async addDebugEntity() {
    ...
    if (this._debugEntity == null) {
      this._debugEntity = new Entity("12");
  ...
  updateDebugEntity() {
    if (this._debugEntity) {
      this._debugEntity.getComponent(Transform).position =
        this.getGlobalPosition().add(this.getShape().position);
    }
  }

  private static getEntityWorldPosition(entity: IEntity): Vector3 | undefined {
    let entityPosition = this.getEntityWorldPositionOrig(entity);

    if (entity) {
      const transform = entity.hasComponent(Transform)
        ? entity.getComponent(Transform)
        : null;
      let originEntity = entity.hasComponent(OriginTransform)
        ? entity.getComponent(OriginTransform).entity
        : null;
      let host = entity.hasComponent(OriginTransform)
        ? entity.getComponent(OriginTransform).host
        : null;
      let scene = entity.hasComponent(OriginTransform)
        ? entity.getComponent(OriginTransform).scene
        : null;
      //TODO put hack in
      if (
        originEntity &&
        ...
        if (
          ...
          (sceneparRotAbsY > 171 && sceneparRotAbsY < 189)
        ) {
          ...
          const parRotAbsY =
            parentRotEuler.y >= 0 ? parentRotEuler.y : parentRotEuler.y + 360; //Math.abs(parentRotEuler.y)
          ...
          if (sceneparRotAbsY > 89 && sceneparRotAbsY < 91) {
            ...
            if (transform != null) {
              if (transform!.scale.y <= 2) {
                extraZBit = 0.8 * transform!.scale.y;
                extraXBit = -0.3 * transform!.scale.y;
              } else {
                extraZBit = 0.6 * transform!.scale.y;
                extraXBit = -0.5 * transform!.scale.y;
              }
            }
            ...
            if (transform != null) {
              if (transform.scale.y < 2) {
                extraZBit = 0.1 * transform.scale.y;
              } else {
                extraZBit = 0.35 * transform.scale.y;
              }

              extraXBit = 0.6 * transform.scale.y;
            }
            ...
            if (transform != null) {
              if (transform.scale.y < 2) {
                ...
                extraZBit = 0.2 * transform.scale.y;
              }
              if (transform.scale.y < 2) {
                extraXBit = 0.8 * transform.scale.y;
              } else {
          ...
          if (!scene180) {
            if (
              (parRotAbsY > 81 && parRotAbsY < 99) ||
              (parRotAbsY > 261 && parRotAbsY < 279)
            ) {
              entityPosition.x = pos0.x + extraXBit;
              if (relativeRotate != null)
                entityPosition.z = pos0.z + relativeRotate.z + extraZBit;
            } else if (parRotAbsY >= 0 && parRotAbsY < 1) {
              //non rotated host
