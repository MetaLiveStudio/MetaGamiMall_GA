...
export default class PoapBooth implements IScript<Props> {
  //model = new GLTFShape("bcebce0f-1873-4148-89bc-24c2ceb9b2cc/models/poap/POAP_dispenser.glb")
  ...
  spawn(host: Entity, props: Props, channel: IChannel) {
    ...
    // POAP BOOTH
    const visible =
      props.visible !== null && props.visible !== undefined && props.visible;
    const enabled =
    ...
    if (
      props.enableTime !== null &&
      props.enableTime !== undefined &&
      props.enableTime.trim() !== ""
    ) {
      const now = new Date();
      const eventTimeDate: Date = new Date(Date.parse(props.enableTime));
      log(
        "poap eventTime now: " +
          now +
          " vs startTime " +
          ...
          now.getTime() +
          " " +
          eventTimeDate.getTime(),
        "notStarted:",
        eventTimeDate.getTime() > now.getTime()
      );
      if (eventTimeDate.getTime() > now.getTime()) {
        ...
        try {
          ...
          log("poap time parse error", e);
        }
      }
    }

    const hoverTextEnabled =
      props.hoverTextEnabled != null && props.hoverTextEnabled != ""
        ? props.hoverTextEnabled
        : "Get Attendance Token";
    const hoverTextDisabled =
      props.hoverTextDisabled != null && props.hoverTextDisabled != ""
        ? props.hoverTextDisabled
        : "Disabled";
    ...
    let POAPBooth = new Dispenser(
      ...
      props.serviceUrl !== undefined &&
      props.serviceUrl.trim() !== ""
        ? props.serviceUrl
        : "poapapi.dcl.guru",
      props.eventName,
      props.clickButton!,
