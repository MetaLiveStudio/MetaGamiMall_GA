...
function applyAudioStreamWorkaroundToEnt(type: string, ent: Entity, audioAttachedToPlayerReadOnly:PBAudioSourceAttachedToPlayer,_removedAudio?:PBAudioSource) {
  if (type === 'enter') {
    if (AudioSource.has(ent)) {
      ...
      const addAudio: PBAudioSource|undefined = SOUNDS_ATTACHED_TO_PLAYER_STORAGE.get(audioAttachedToPlayerReadOnly.id);
    ...
    //exit 
    const removedAudio: PBAudioSource|null = AudioSource.deleteFrom(ent);
