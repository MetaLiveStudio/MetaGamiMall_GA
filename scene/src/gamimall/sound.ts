...
export const finishSound1 = new AudioClip("sounds/level-completed.mp3");
//export const finishSound2 = new AudioClip("sounds/deja-vu.mp3");
export const countdownRestartSound = new AudioClip(
  "sounds/countdown-restart.mp3"
);
...
function play(
  ...
  const entity = new Entity("225");
  const audio = new AudioSource(clip);
  log("play clip at volume ", volume);
  audio.volume = volume;
...
export function playOnceRandom(clips: AudioClip[], volume: number = 1) {
  ...
  playOnce(clips[Math.floor(Math.random() * clips.length)], volume);
}
