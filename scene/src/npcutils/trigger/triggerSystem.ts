...
export class NPCTriggerSystem implements ISystem {
  ...
  private static removeTriggerFromSystem(wrapper: TriggerWrapper) {
    ...
    for (let i = 0; i < activeCollisions.length; i++) {
      let activeCollisionHasTrigger = !(
        activeCollisions[i] ===
          NPCTriggerSystem._instance?._cameraTriggerWrapper ||
  ...
  private checkCollisionAgainstCamera(wrapper: TriggerWrapper) {
    ...
    let areColliding = NPCTriggerSystem.areColliding(
      wrapper,
    ...
    for (const key in this._triggers) {
      if (this._triggers.hasOwnProperty(key)) {
        if (key != wrapper.uuid && this._triggers[key].trigger.enabled) {
          if (
            NPCTriggerSystem.canTriggersCollide(wrapper, this._triggers[key])
          ) {
...
class TriggerWrapper {
  ...
  addDebugEntity() {
    ...
    if (this._debugEntity == null) {
      this._debugEntity = new Entity("244");
...
export class NPCTriggerComponent {
  ...
  constructor(shape: TriggerBoxShape | TriggerSphereShape, data?: TriggerData) {
    ...
    this.triggeredByLayer =
      data && data.triggeredByLayer ? data.triggeredByLayer : 0;
    this.onTriggerEnter =
      data && data.onTriggerEnter ? data.onTriggerEnter : undefined;
    this.onTriggerExit =
      data && data.onTriggerExit ? data.onTriggerExit : undefined;
    this.onCameraEnter =
      data && data.onCameraEnter ? data.onCameraEnter : undefined;
    this.onCameraExit =
      data && data.onCameraExit ? data.onCameraExit : undefined;
