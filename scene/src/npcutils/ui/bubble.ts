...
export class DialogBubble {
  ...
  constructor(parent: Entity, height?: number, sound?: string) {
    ...
    // Root
    this.rootEntity = new Entity("245");
  ...
  public closeDialogEndAll(): void {
    if (this.isBubleOpen && WorldDialogTypeInSystem._instance) {
      if (WorldDialogTypeInSystem._instance.Dialog == this) {
        WorldDialogTypeInSystem._instance.done = true;
        WorldDialogTypeInSystem._instance.Dialog = undefined;
      }
      this.isBubleOpen = false;
  ...
  public skipDialogs() {
    ...
    while (this.NPCScript[this.activeTextId]) {
      let tigger = this.NPCScript[this.activeTextId].triggeredByNext;
      if (tigger) {
        tigger();
      }
  ...
  update(dt: number) {
    ...
    if (this.showing) {
      if (this.timer > this.timeOn) {
        ...
        this.done = true;
        this.timer = 0;
        if (this.window) this.window.next();
      }
