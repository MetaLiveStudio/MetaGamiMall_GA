...
import { ButtonDataType, WearableOptionsType } from "../types";
import { buy, buyVC } from "../blockchain/index";
...
export class WearableMenuItem extends MenuItem {
  ...
  buyVCButton: Entity;
  buyVCButtonText: TextShape;
  buyVCButtonTextRoot: Entity;
  ...
  constructor(
    ...
    _alphaTexture: Texture | undefined,
    _collection: any,
    _item: any,
    options: WearableOptionsType
  ) {
    super(parent.name + "-child");
    this.parent = parent;
    //this.addComponent(new Transform({position: new Vector3(-1.4,1,0), rotation: Quaternion.Euler(0,0,0), scale: new Vector3(0,0,0)}));
    this.addComponent(new Transform({}));
    //this.setParent(parent)
    //hud.attachToEntity(this)

    const cardData = options.cardData;
    if (!cardData) throw new Error("cardData required!!");

    this.buttonData = cardData.buttonData; //{text:"Claim",hoverTextBuyable:"BUY WEARABLE"}
    log(parent.name, "this.buttonData", this.buttonData);

    this.scale = new Vector3(1, 0.5, 1);
    this.scaleMultiplier = 1.2;
    this.defaultItemScale = new Vector3(1, 1, 1);
    this.cardOffset = cardData.cardOffset
      ? cardData.cardOffset
      : Vector3.Zero(); //new Vector3(-1.5, 0.5, -0);
    this.currencyText = cardData.currencyText ? cardData.currencyText : "MANA";
    ...
    if (ethClean(_item.price) === "0") this.priceTextShape.value = "Free";
      ...
      this.priceTextShape.value =
        ethClean(_item.price) + " " + this.currencyText;
    ...
    // -- BUY BUTTON
    // -- BUY BUTTON
    const createdButton = createBuyButton(
      "buy",
      this,
      this.detailsCard,
      this.cardOffset,
    ...
    this.buyButton = createdButton.buyButton;
    this.buyButtonText = createdButton.buyButtonText;
    this.buyButtonTextRoot = createdButton.buyButtonTextRoot;

    const createdVCButton = createBuyButton(
      "buyVC",
      this,
      this.detailsCard,
      this.cardOffset,
    ...
    this.buyVCButton = createdVCButton.buyButton;
    this.buyVCButtonText = createdVCButton.buyButtonText;
    this.buyVCButtonTextRoot = createdVCButton.buyButtonTextRoot;
    ...
    this.updateItemInfo(_collection, _item);
    //this.select()
    //log('we got here')
  }
  ...
  boughtOne(): void {
    ...
    if (+available > 0)
      this.availableText.value = +available - 1 + "/" + maxSupply;
  }
  ...
  updateItemInfo(_collection: any, _item: any) {
    ...
    //update buy button
    this.buyButtonText.value = this.buttonData.text;
    this.buyButton.getComponent(OnPointerDown).callback = async () => {
      buy(_collection.id, _item.blockchainId, _item.price, this);
    };
    this.buyButton.getComponent(OnPointerDown).hoverText =
      this.buttonData.hoverTextBuyable;
    this.buyButton.getComponent(OnPointerDown).button = ActionButton.POINTER;

    //update buy button
    this.buyVCButtonText.value = this.buttonData.text;
    this.buyVCButton.getComponent(OnPointerDown).callback = async () => {
      buyVC(_collection.id, _item.blockchainId, _item.price, undefined, this);
    };
    this.buyVCButton.getComponent(OnPointerDown).hoverText =
      this.buttonData.hoverTextBuyable + "VC";
    this.buyVCButton.getComponent(OnPointerDown).button = ActionButton.POINTER;
  ...
  select() {
    if (!this.selected) {
      log("we selected the item");
      // engine.addEntity(this.detailsRoot)
      this.setParent(this.parent);
      this.selected = true;
      this.buyButton.getComponent(AnimatedItem).isHighlighted = true;
      if (this.buyVCButton.hasComponent(AnimatedItem))
        this.buyVCButton.getComponent(AnimatedItem).isHighlighted = true;
  ...
  deselect(_silent?: boolean) {
    ...
    if (this.buyVCButton.hasComponent(AnimatedItem))
      this.buyVCButton.getComponent(AnimatedItem).isHighlighted = false;
...
function createBuyButton(
  ...
  const vcBuyTye = type == "buyVC";

  const buyButton = new Entity();
  buyButton.addComponent(
    new Transform({
      position: new Vector3(cardOffset.x, cardOffset.y - 0.2, cardOffset.z),
      scale: new Vector3(0.1, 0.1, 0.1),
    })
  );
  buyButton.addComponent(resource.buyButtonShape);
  buyButton.setParent(parent);
  buyButton.addComponent(
    new AnimatedItem(
      {
  ...
  const buyButtonTextRoot = new Entity();
  const buyButtonText = new TextShape();

  buyButtonText.color = Color3.FromHexString("#FFFFFF");
  buyButtonText.font = new Font(Fonts.SansSerif_Heavy);
  buyButtonText.hTextAlign = "center";

  buyButtonTextRoot.addComponent(buyButtonText);
  buyButtonTextRoot.addComponent(
    new Transform({
      position: new Vector3(0, 0.0, -0.05),
      scale: new Vector3(0.1, 0.1, 0.1),
    })
  ...
  buyButtonTextRoot.setParent(buyButton);
  buyButtonText.value = buttonData.text;
  if (_item.available > 1) {
    buyButton.addComponent(
      new OnPointerDown(
        async () => {
          //buy(_collection.id, _item.blockchainId, _item.price, caller);
          buyVC(_collection.id, _item.blockchainId, _item.price, undefined, caller);
        },
        {
          button: ActionButton.POINTER,
          hoverText: buttonData.hoverTextBuyable + buyType,
        }
  ...
  return {
    buyButton: buyButton,
    buyButtonTextRoot: buyButtonTextRoot,
