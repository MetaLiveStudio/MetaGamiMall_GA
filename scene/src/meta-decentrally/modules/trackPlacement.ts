...
import { getOrCreateGLTFShape, getOrCreateMaterial } from 'src/meta-decentrally/resources/common';
...
import { SceneEntity, VisibilityStrategyEnum, VisibleChangeType } from './scene/subScene';
...
import { SPARKLE_MANAGER } from 'src/gamimall/coin';


//let trackEdgeShape =  new GLTFShape('models/racing-models/track_edge.glb')
//let trackEdgeShape =  new GLTFShape('models/racing-models/track_edge_sand.glb')
//let trackEdgeShape =  new GLTFShape('models/racing-models/track_edge_meta.glb')
let finishFlagShape =  new GLTFShape('models/racing-models/finish_flag.glb')
//let treeShape =  new GLTFShape('models/racing-models/tree.glb')
//let rockShape =  new GLTFShape('models/racing-models/rock.glb')
//let metaLampShape =  new GLTFShape('models/racing-models/meta_lamp.glb')
//let metaBuildingShape =  new GLTFShape('models/racing-models/meta_building.glb')
...
export class TrackData{
  ...
  trackPathDistances:number[] = [] 

  fullDistance = 0
  trackPathFullDistanceToID:number[] = []
      ...
      if(!featCopy.shape){
        ...
        switch(featCopy.type){
          ...
          case "slow-down":
            ...
            break
          case "coin-gc":
            themeTrackFeat = level.theme.trackFeature.coinGc
            break
          case "coin-mc":
            themeTrackFeat = level.theme.trackFeature.coinMc
            break
          case "material-a":
...
const featureGroup = engine.getComponentGroup(TrackFeatureComponent)
  ...
  const triggerOnCameraEnter = ()=>{
    ...
    switch(trackFeature.type){
      ...
      case 'material-a':
        ...
        let trackFeatComp:TrackFeatureComponent = undefined//TODO lookup active live trackfeatures
        for (let entity of featureGroup.entities) 
        {
          const trackFeatCompInst = entity.getComponent(TrackFeatureComponent)
          if(!entity.alive){
            log("projectile","not alive skipping",trackFeatCompInst.name)
            return
          }
          if(!entity.hasComponent(Transform)){
            log("projectile","missing transform skipping",trackFeatCompInst.name)
            return
          }
          if(trackFeatCompInst.name == trackFeature.name){
            trackFeatComp = trackFeatCompInst
            break;
          }
        }
        if(trackFeatComp !== undefined){
          log("trackFeatComp found match for ",trackFeatComp.name)
        }else{
          log("trackFeatComp filed to find match for ",trackFeature.name)
        }
        ...
        if(trackFeatComp !== undefined && trackFeatComp.sceneEnts){
          log("trackFeatComp.player","hit.feature",trackFeatComp.name,"new activate time",newActivateTime,trackFeatComp.sceneEnts)
          trackFeatComp.activateTime = newActivateTime
          
          const sparklePos = trackFeatComp.sceneEnts[0].entity.getComponent(Transform).position.clone();
          sparklePos.y -= 1;
          ...
          for(const p in trackFeatComp.sceneEnts){
            log("trackFeatComp.player","hit.feature",trackFeatComp.name,"hiding",trackFeatComp.sceneEnts[p].entity.name)
