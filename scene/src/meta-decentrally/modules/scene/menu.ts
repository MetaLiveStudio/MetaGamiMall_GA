...
import { GetLeaderboardResult, GetPlayerCombinedInfoResultPayload, StatisticValue } from "src/gamimall/playfab_sdk/playfab.types"
import * as PlayFabSDK from  'src/gamimall/playfab_sdk/index'
...
let leaderboardShape = new GLTFShape('models/racing-models/menu/menu_bg.glb')
...
export class LeaderBoardManager {
  ...
  updateBoardWithLoaderBoardId(leaderboard:Leaderboard,menuItmLabl:string,statId:string,scalar:number,fractionDigits:number){
    ...
    if (isNull(GAME_STATE.playerState.playFabLoginResult)) {
      leaderboard.updateAllRows( "Login First",""  )
      REGISTRY.ui.openloginGamePrompt();
      log("User not logged in yet")
      return
    }
