...
const clickDebounce = new IntervalUtil(CONFIG.GAME_OTHER_ROOM_DISCONNECT_TIMER_WAIT_TIME + 1000,'abs-time')
...
export class LobbyScene extends SubScene{
  ...
  onInit(scene:SubScene){
    ...
    /*
    const baseShape = new GLTFShape('models/lobby_building.glb')//new GLTFShape('models/racing-models/lobby/lobby_building.glb')
  ...
  private spawnLeaderboards(leaderboardManager: LeaderBoardManager) {
    const level1 = this.level1 = leaderboardManager.createLeaderboard(this.rootEntity, levelManager.levels[0].name, levelManager.levels[0].id, SceneData.menuPositions[1].position, SceneData.menuPositions[1].rotation, 1.4);
    ...
    const boards = this.leaderboards = [level1/*, level1, level0b, level1b*/];
      ...
      board.addMenuItem(
        new LevelMenuItem(
          ...
          () => {
            ...
            if(clickDebounce.update()){
              log("gorace.debounce click.fired")
              utils.setTimeout(300, () => {
                Constants.SCENE_MGR.goRace();
              });
            }else{
              log("gorace.debounce click.ignored")
            }
            //deselect
          },
          {
