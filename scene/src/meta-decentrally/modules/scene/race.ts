...
const raceGroundShapes = [groundShape,RACING_PLAYER_STANDON_GROUND_COLLIDER,PREVENT_PLAYER_JUMP_COLLIDER,LOWER_PLAYER_SLOWLY_COLLIDER]
...
export class RacingScene extends SubScene{
  ...
  firstTimeEnterCarWorkaroundVisibleBloomOnMove() {
    ...
    this.car.enableAllCarCollider()//enable them all
    this.car.disableTopCollider()//then disable top
  ...
  toggleColliders(val:boolean){
     
    for(const p in raceGroundShapes){
      if(raceGroundShapes[p].visible != val ) raceGroundShapes[p].visible = val
      if(raceGroundShapes[p].withCollisions != val ) raceGroundShapes[p].withCollisions = val
    } 
     
    //nearbySpawningPad2.activatePlatform()
    try{
      if(val){ 
        log("toggleColliders showing car")
        this.car.show()
        
        if(this.car) this.car.enableAllCarCollider()
      }else{
        log("toggleColliders hiding car")
        this.car.hide()
         
        if(this.car) this.car.disableAllCarCollider()
        nearbySpawningPad2.deActivatePlatform()
      }
    }catch(e){
      log("toggleColliders errored",e)
      //errors(e)
    }
  }
