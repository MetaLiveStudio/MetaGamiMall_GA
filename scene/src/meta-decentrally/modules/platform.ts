...
export class BoxedInPlatflormUsingModel{
  ...
  constructor(parent: Entity, bDebugMode: boolean = false) {
    ...
    boxInEnt.addComponentOrReplace( new GLTFShape( 'models/racing-models/car_collider_no_top.glb')) //
  }
  activatePlatform() {
    this.PREVENT_PLAYER_JUMP_COLLIDER.withCollisions = true
    if(!this.boxInEnt.alive) engine.addEntity(this.boxInEnt)
    //this.updatePosToPlayer()
  }
  deActivatePlatform() {
    if(this.boxInEnt.alive) engine.removeEntity(this.boxInEnt)
    this.PREVENT_PLAYER_JUMP_COLLIDER.withCollisions = false
  }
}
...
export class RocketBoard extends Entity {
  ...
  constructor(startPos: Vector3) {
    ...
    this.addComponent(new GLTFShape('models/racing-models/rocketBoard.glb'))

    this.rocketFlames = new Entity()
    this.rocketFlames.addComponent(
      new Transform({ scale: new Vector3(0, 0, 0) })
    )
    this.rocketFlames.addComponent(new GLTFShape('models/racing-models/rocketFlames.glb'))
    this.rocketFlames.setParent(this)

    this.rocketBoosterSound = new Sound(
      new AudioClip('sounds/racing/rocketBooster.mp3'),
      true
    )
  }
