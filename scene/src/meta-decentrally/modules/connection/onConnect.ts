...
import { levelManager } from 'src/meta-decentrally/tracks/levelManager'
...
import { fetchPlayerCombinedInfo } from 'src/gamimall/login-flow';
...
function updatePlayerInventory(inventory:clientState.PlayerInventoryState){
    GAME_STATE.setGameCoinGCValue(inventory.coinGcCount)
    GAME_STATE.setGameCoinMCValue(inventory.coinMcCount)
}
...
export function onDisconnect(room: Room,code?:number) {
    ...
    if(isNormalDisconnect(code)){
        GAME_STATE.setGameConnected('disconnected',"onDisconnect")
    }else{
        GAME_STATE.setGameConnected('disconnecting',"onDisconnect")
    }
...
function onLevelConnect(room: Room<clientState.RaceRoomState>) {
    ...
    room.state.players.onAdd = (player: clientState.PlayerState, sessionId: string) => {
        ...
        player.listen("inventory", (inventory: clientState.PlayerInventoryState) => {
            log("player.listen.inventory",inventory)
        ...
        player.inventory.onChange = (changes:DataChange<any>[])=>{
            log("player.listen.inventory.onChange",changes)
