...
import { MovesWithWorld, VehicleObject } from 'src/meta-decentrally/components/moveWithWorld';
...
//let carBodyShape01 =  new GLTFShape('models/racing-models/car.glb')
let skidmarkShape =  new GLTFShape('models/racing-models/skidmark.glb')
let fireShape =         new GLTFShape('models/racing-models/fire.glb')

const carPosition = new Vector3(scene.center.x,scene.center.y+0.02,scene.center.z)

const CAR_COLLIDER_POSITION = new Transform({
...
const carDriverIdle = "00LilDogeIdle"
const carDriverDrive = "00LilDogeRun"
...
export class BaseCar extends EntityWrapper{
  ...
  enableAllCarCollider(){
    this.enableTopCollider()
    if(this.carCollider ) {
      if(!this.carCollider.alive && this.carCollider.getParent().alive){
        log("enableAllCarCollider.added to engine")
      }else{
        log("enableAllCarCollider.parent was not alive could not add to engine")
      }
      ////calling this bombs after second time,  not sure why
      //engine.addEntity(this.carCollider)
      //instead going to set it way way high up then put it back
      this.carCollider.getComponent(Transform).position.copyFrom(CAR_COLLIDER_POSITION.position)
    }
  }
  disableAllCarCollider(){
    this.disableTopCollider()
    //calling this bombs after second time,  not sure why
    //if(this.carCollider && this.carCollider.alive) engine.removeEntity(this.carCollider)
    //instead going to set it way way high up then put it back
    this.carCollider.getComponent(Transform).position.y = 999
  }
  enableTopCollider(){
  ...
  updateCarData(carData:CarData){
    ...
    this.driverEntity.addComponentOrReplace(new Animator())  
    this.driverEntity.getComponent(Animator).addClip(new AnimationState(carDriverDrive,{looping:true}))
    this.driverEntity.getComponent(Animator).addClip(new AnimationState(carDriverIdle,{looping:true}))
    this.driverEntity.getComponent(Animator).getClip(carDriverIdle).play()
  ...
  updateDriverRotation(){
    ...
    if(this.driver.currentSpeed > 0){
      this.driverEntity.getComponent(Animator).getClip(carDriverDrive).play()
    }else{
      this.driverEntity.getComponent(Animator).getClip(carDriverIdle).play()
    }
    /*    
...
export class Car extends BaseCar {
  ...
  updateCarData(carData:CarData){
    ...
    this.carCollider.addComponentOrReplace( new GLTFShape(CONFIG.ENABLE_BLOOM_VISIBLE_WORKAROUNDS ? 'models/racing-models/car_collider_no_top.glb':'models/racing-models/car_collider.glb')) //
    ...
    if(CONFIG.ENABLE_BLOOM_VISIBLE_WORKAROUNDS){
      ...
      /*
       this.carColliderTop.addComponentOrReplace( new GLTFShape('models/racing-models/car_collider_top_only.glb')) //
