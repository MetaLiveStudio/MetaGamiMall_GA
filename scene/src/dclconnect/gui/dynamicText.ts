...
export class DynamicText implements ISystem {
  ...
  fadeTo(to: number, duration: number) {
    ...
    this.animations.opacity = new DynamicTextAnimationNumber({
      from,
      to,
      duration,
    });
  ...
  moveTo(to: Vector2, duration: number) {
    let { positionX, positionY } = this.text;
    let from = new Vector2(parseInt(positionX + ""), parseInt(positionY + ""));
    this.animations.position = new DynamicTextAnimationVector2({
      from,
      to,
      duration,
    });
  ...
  resizeTo(to: Vector2, duration: number) {
    let { width, height } = this.text;
    let from = new Vector2(parseInt(width + ""), parseInt(height + ""));
    this.animations.size = new DynamicTextAnimationVector2({
      from,
      to,
      duration,
    });
  ...
  private handleSize(dt: number) {
    if (this.animations.size) {
      ...
      if (
        parseInt(this.text.width + "") == options.to.x &&
        parseInt(this.text.height + "") == options.to.y
      ) {
  ...
  private handlePosition(dt: number) {
    if (this.animations.position) {
      ...
      if (
        ...
        let lerpTime = Scalar.Clamp(timer / options.duration, 0, 1);
        this.text.positionX = Scalar.Lerp(
          options.from.x,
          options.to.x,
          lerpTime
        );
        this.text.positionY = Scalar.Lerp(
          options.from.y,
          options.to.y,
          lerpTime
        );
