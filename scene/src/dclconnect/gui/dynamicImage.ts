...
export class DynamicImage implements ISystem {
  ...
  fadeTo(to: number, duration: number) {
    if (this.animate) {
      ...
      this.animations.opacity = new DynamicAnimationNumber({
        from,
        to,
        duration,
      });
  ...
  moveTo(to: Vector2, duration: number, isPercent: boolean = false) {
    if (this.animate) {
      ...
      this.animations.position = new DynamicAnimationVector2({
        from,
        to,
        duration,
        isPercent,
  ...
  resizeTo(to: Vector2, duration: number, isPercent: boolean = false) {
    if (this.animate) {
      ...
      let from = new Vector2(parseInt(width + ""), parseInt(height + ""));
      this.animations.size = new DynamicAnimationVector2({
        from,
        to,
        duration,
        isPercent,
  ...
  moveSourceTo(to: Vector2, duration: number) {
    if (this.animate) {
      ...
      this.animations.sourcePosition = new DynamicAnimationVector2({
        from,
        to,
        duration,
      });
  ...
  resizeSourceTo(to: Vector2, duration: number) {
    if (this.animate) {
      ...
      this.animations.sourceSize = new DynamicAnimationVector2({
        from,
        to,
        duration,
      });
  ...
  private handleSize(dt: number) {
    if (this.animations.size) {
      ...
      if (
        parseInt(this.image.width + "") == options.to.x &&
        parseInt(this.image.height + "") == options.to.y
      ) {
  ...
  private handlePosition(dt: number) {
    if (this.animations.position) {
      ...
      if (
        parseInt(this.image.positionX + "") == options.to.x &&
        parseInt(this.image.positionY + "") == options.to.y
      ) {
  ...
  private handleSourceSize(dt: number) {
    if (this.animations.sourceSize) {
      ...
      if (
        ...
        let lerpTime = Scalar.Clamp(timer / options.duration, 0, 1);
        this.image.sourceWidth = Scalar.Lerp(
          options.from.x,
          options.to.x,
          lerpTime
        );
        this.image.sourceHeight = Scalar.Lerp(
          options.from.y,
          options.to.y,
          lerpTime
        );
  ...
  private handleSourcePosition(dt: number) {
    if (this.animations.sourcePosition) {
      ...
      if (
        ...
        let lerpTime = Scalar.Clamp(timer / options.duration, 0, 1);
        this.image.sourceLeft = Scalar.Lerp(
          options.from.x,
          options.to.x,
          lerpTime
        );
        this.image.sourceTop = Scalar.Lerp(
          options.from.y,
          options.to.y,
          lerpTime
        );
  ...
  update(dt: number) {
    if (this.checkIsAnimating()) {
      ...
      if (this.animations.sourceSize) {
        this.handleSourceSize(dt);
      }
      if (this.animations.sourcePosition) {
        this.handleSourcePosition(dt);
      }
    } else {
      this.disable();
    }
