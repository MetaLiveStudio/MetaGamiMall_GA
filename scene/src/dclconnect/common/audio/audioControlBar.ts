...
export class AudioControlBar extends Entity {
  ...
  constructor(
    private source: AudioStream | AudioSource | VideoTexture,
    ...
    this.audioIcon = source instanceof AudioSource ? audioIcon() : videoIcon();
    this.audioMuteIcon = audioMuteIcon();
    this.audioMutedIcon = audioMutedIcon();
    this.audioIcon.image.onClick = new OnClick(() => {
      this.setVolumePreference(10);
    });
    this.audioMuteIcon.image.onClick = new OnClick(() => {
      log("mute");
      this.toggleMute();
    });
    this.audioMutedIcon.image.onClick = new OnClick(() => {
      log("muted");
      this.toggleMute();
    });
    this.addComponent(source);
    this.updateGUI();
    this.setVolume(startMuted ? 0 : defaultVolume);
    if (startMuted) {
      this.mute();
    }
    for (let i = 0; i < 10; i++) {
