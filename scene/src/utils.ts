...
import { createFont } from "./gamimall/resources";
...
export const getEntityBy = (
  ...
  let val: IEntity[] | undefined = undefined;
...
export interface ILazyMap<K, T> {
  ...
  delete(name: string): T | undefined;
}
...
export function computeFaceAngle(
  ...
  if (lockX || lockY || lockZ) {
    ...
    if (canUseEuler) {
      //tthe builder is running sub 6.6.4 (6.6.3 i think) and has bug with euler angle conversion
      ...
      //works in builder but not sure this is valid rotation lock logic
...
export function computeMoveVector(
  ...
  lockY: boolean,
  lockZ: boolean,
  percentOfDistanceToTravel: float,
  moveNoCloserThan: float
) {
  ...
  } else if (distanceWhole > 0.0001) {
    percentStopDistToUse = distanceDelta / distanceWhole;
    percentStopDistToUse = Math.min(
      percentStopDistToUse,
      percentOfDistanceToTravel
    );
...
export function createTextShape(
  ...
  const textShape = new TextShape(strText);

  //text.height
  textShape.fontSize = fontSize ? fontSize : 1;
  textShape.color = textColor;
  textShape.hTextAlign = hTextAlign ? hTextAlign : "left";
  textShape.vTextAlign = "center";
  //text.resizeToFit=true
  textShape.font = createFont(textFont);

  textShape.width = 10;
  textShape.height = 0;
  //text.hTextAlign = 'center'
  //isPickable not exist anymore
  //textShape.isPickable = false; //non cursor blocking
...
export function removeFromEngine(entity: IEntity, delay?: number) {
  if (notNull(entity)) {
    const delayVal = delay !== undefined ? delay : -1;
    if (delayVal > 0) {
